sap.ui.define([
    ...
    return PluginControllerExtension.extend("rits.phaseDetailplugin.plugin.phaseDetailExtension.assemblyPointExtensionProvider.PluginEventExtension", {
        ...
        generateAssemblyDataFields: function(oComponentData) {
            console.log("=== generateAssemblyDataFields called ===");
        
            var bomComponentRef = oComponentData.bomComponentRef;
            var bomParts = bomComponentRef.split(",");
            var bomValue = bomParts[1];
            var bomVersion = bomParts[3];
        
            console.log(bomValue);
        
            var oForm = this.getController().byId("assyPointFindForm");
        
            function createLabelInputPair(oForm, labelText, inputId, inputValue) {
                var oLabel = new sap.m.Label({ text: labelText });
            ...
            createLabelInputPair(oForm, "BOM", "bomInputadd", bomValue);
            createLabelInputPair(oForm, "BOM Version", "bomversionadd", bomVersion);
